...
def process_file(file_path: Path, fix: bool, check_all: bool = False) -> Tuple[int, List[str]]:
  ...
  for line_num, line in enumerate(content.splitlines(keepends=True), 1):
    ...
    for orig, repl in REPLACEMENTS.items():
      if orig in new_line:
        # Don't auto-replace inside the specific rule definition line explaining U+276F
        ...
        issues.append(f"{file_path}:{line_num}: Found {count}x {char_name} -> suggest '{repl}'")
        if fix:
      ...
      for col_num, ch in enumerate(line, 1):
        if ord(ch) > 127 and ch not in REPLACEMENTS:
          if not check_all and ch in ALLOWED_BOX_DRAWING:
            continue
          issues.append(f"{file_path}:{line_num}:{col_num}: Non-basic ASCII U+{ord(ch):04X} ('{ch}')")
...
def main():
  parser = argparse.ArgumentParser(description="Lint and fix non-basic ASCII characters in Markdown")
  parser.add_argument("paths", nargs="+", type=Path, help="Files or directories to inspect")
  parser.add_argument("--fix", action="store_true", help="Automatically replace common homoglyphs and dashes")
  parser.add_argument("--check-all", action="store_true", help="Also flag standard box-drawing tree characters")
